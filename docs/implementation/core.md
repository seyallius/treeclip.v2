...
clipboard operations, editor interactions, error handling, glob expansion, and UI utilities.
...
- **`errors` (`core/errors.rs`):** Defines custom error types using `thiserror` and provides guidance on error handling
  patterns within the application. It uses `anyhow` for context propagation. Includes `GlobPatternError` for glob input
  expansion errors.
...
- **`glob` (`core/glob.rs`):** Expands glob patterns in input paths into concrete file/directory paths. Supports
  git-style glob semantics including `*`, `**`, `?`, `[...]`, and `{...}`. Unmatched patterns produce errors to prevent
  silently generating empty output.
...
- **Glob Expansion:** The `glob` module expands input path glob patterns (e.g., `src/**/*.rs`) into concrete paths
  before traversal begins. This allows users to select specific files without manually listing each one.
...
- **Error Handling:** The `errors` module defines specific error types (`ClipboardError`, `FileSystemError`,
  `GlobPatternError`, etc.) and demonstrates how to chain them with context using `anyhow` for better debugging and
  user feedback.
